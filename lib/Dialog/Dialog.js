  ...
  useUpdateEffect.useUpdateEffect(function () {
    ...
    if (initialFocusRef && initialFocusRef.current) {
      ...
      if (tabbable) {
        ...
        process.env.NODE_ENV !== "production" ? warning.warning(dialog.tabIndex === undefined || dialog.tabIndex < 0, "Dialog", "It's recommended to have at least one tabbable element inside dialog. The dialog element has been automatically focused.", "If this is the intended behavior, pass `tabIndex={0}` to the dialog element to disable this warning.", "See https://reakit.io/docs/dialog") : void 0;
      }
    }
...
function usePortalRef(dialogRef, options) {
  ...
  React.useEffect(function () {
    ...
    portalRef.current = dialog.closest(Portal.Portal.__selector);
...
function useEventListenerOutside(targetRef, disclosureRef, nestedDialogs, event, listener, shouldListen) {
  ...
  React.useEffect(function () {
    ...
    var handleEvent = function handleEvent(e) {
      ...
      var target = e.target;
      process.env.NODE_ENV !== "production" ? warning.warning(!element, "Dialog", "Can't detect events outside dialog because `ref` wasn't passed to component.", "See https://reakit.io/docs/dialog") : void 0; // Click inside
...
var useDialog = createHook.createHook({
  ...
  useProps: function useProps(options, _ref2) {
    ...
    var wrapChildren = React.useCallback(function (children) {
      if (options.unstable_portal) {
        return React.createElement(Portal.Portal, null, wrap(children));
      }
    ...
    return _rollupPluginBabelHelpers._objectSpread2({
      ...
      unstable_wrap: usePipe.usePipe(wrapChildren, htmlWrap)
    }, htmlProps);
  }
});
